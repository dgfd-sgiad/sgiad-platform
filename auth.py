...
SIGNUP_ENABLED = os.environ.get('SIGNUP_ENABLED', 'false').lower() in ('1', 'true', 'yes', 'oui')
# Domaines d'email autorises pour l'inscription (separes par des virgules).
# Vide = tous les domaines sont acceptés.
...
def login():
    ...
    try:
        ...
        log_session(request.remote_addr, resp.user.email or email)
...
def signup():
    ...
    Si ALLOWED_EMAIL_DOMAINS est vide, toutes les adresses email sont autorisées.
    """
    try:
        ...
        if not _email_domain_allowed(email):
            ...
            return jsonify({'error': 'Inscription réservée aux emails autorisés.'}), 403
        if len(password) < 8:
