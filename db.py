...
Variables d'environnement requises :
    ...
    SUPABASE_KEY  = eyJhbGciOi...  (service_role recommandee pour le backend Flask)
"""
...
def get_session_stats(ip_address: str) -> dict:
    """Retourne les statistiques de session sans enregistrer de nouvelle visite."""
    sb = get_supabase()
    resp = sb.table('session_log').select(
        'derniere_connexion', 'total_connexions'
    ).order('id', desc=True).limit(1).execute()

    total = resp.data[0]['total_connexions'] if resp.data else 0
    prev = None
    if resp.data:
...
def log_session(ip_address: str, user_email: str = None) -> dict:
    """Enregistre une connexion authentifiée et retourne les infos de session."""
    sb = get_supabase()
