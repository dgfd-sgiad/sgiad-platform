...
body{margin:0;background:var(--canvas);color:var(--ink);font-family:'Archivo',system-ui,sans-serif;font-size:14px}
...
nav .tab{background:none;border:0;cursor:pointer;font:inherit;padding:0 2px 10px;color:var(--ink3)}
nav .tab.on{color:var(--dec);font-weight:600;border-bottom:2px solid var(--dec)}
...
.fig:nth-child(1),.fig:nth-child(2){border-top:0}.fig:nth-child(odd){border-left:0}
...
th.sortable{cursor:pointer}th.sortable:hover{color:var(--dec)}
...
.pill.ok{background:var(--oksoft);color:#265A50}.pill.vig{background:var(--vigsoft);color:#8A6416}.pill.inf{background:var(--infosoft);color:#2C4459}.pill.ris{background:var(--risquesoft);color:#8E3E2B}
...
#proj-search{width:100%;margin:10px 0 4px;padding:8px 10px;border:1px solid var(--line);border-radius:6px;font:inherit;font-size:12px}
...
<button class="tab" data-v="projets">▣ Projets</button>
<button class="tab" data-v="recos">☷ Recommandations</button>
<button class="tab" data-v="revues">▢ Revues &amp; missions</button>
<button class="tab" data-v="partenaires">♧ Partenaires</button>
</nav>
</div></header>
...
<div class="card"><div class="cardhead"><h4>Partenaires : volume engagé contre taux</h4><span class="unit">Moyenne du<br>portefeuille</span></div><div id="par-rows"></div><div class="note" id="i-par"></div></div>
</div>
<div class="grid" style="margin-top:14px">
...
<div class="wrap" id="view-projets" hidden>
<div class="card"><div class="cardhead"><h4>Projets en cours du périmètre</h4><span class="unit" id="proj-count"></span></div>
<input id="proj-search" placeholder="Rechercher un projet ou un code…">
<table><thead><tr><th class="sortable" data-k="nom">Projet / Code</th><th>Secteur</th><th>Partenaire</th><th class="sortable tnum" data-k="prev">Engagé</th><th class="sortable tnum" data-k="dec">Décaissé</th><th class="sortable tnum" data-k="taux">Taux</th><th class="sortable tnum" data-k="fin">Clôture</th></tr></thead><tbody id="t-projets"></tbody></table></div>
</div>

<div class="wrap" id="view-recos" hidden>
<div class="card"><div class="cardhead"><h4>Recommandations de revues</h4><span class="unit" id="reco-count"></span></div>
<table><thead><tr><th>Action attendue</th><th>Partenaire</th><th class="tnum">Échéance</th><th class="tnum">Retard</th><th>Statut</th></tr></thead><tbody id="t-recos"></tbody></table></div>
</div>

<div class="wrap" id="view-revues" hidden>
<div class="grid">
<div class="card"><div class="cardhead"><h4>Revues programmées</h4><span class="unit">Prochains mois</span></div><ul class="tl" id="tl-revues2"></ul></div>
<div class="card"><div class="cardhead"><h4>Missions prioritaires (justification &amp; clôture)</h4><span class="unit">Solde à verser</span></div><ul class="tl" id="mis-list"></ul></div>
</div>
</div>

<div class="wrap" id="view-partenaires" hidden>
<div class="grid" id="par-cards"></div>
</div>
...
let D=null, RECOS=[], RECO_FULL=null, REVUES_FULL=null, CUR='dash';
...
function renderRail(a){
 ...
 $('rail-list').innerHTML=rows.map(p=>{const late=p.fin&&p.fin<='2027-08-28';return '<div class="rrow"><div class="nm">'+p.nom+'</div><div class="meta"><span>'+D.secteurs[p.s]+'</span><span class="fin'+(late?' late':'')+'">Clôture : '+(p.fin?p.fin.slice(8,10)+'/'+p.fin.slice(5,7)+'/'+p.fin.slice(0,4):'—')+'</span></div><div class="meta"><span>'+D.partenaires[p.p]+'</span><span>'+NB0.format(p.prev?p.dec/p.prev*100:0)+' % décaissé</span></div></div>';}).join('')||'<div class="empty">Aucun projet.</div>';
}
let PROJ_Q='',PROJ_K='prev',PROJ_DIR=-1;
function pval(p,k){if(k==='taux')return p.prev?p.dec/p.prev*100:0;if(k==='fin')return p.fin||'9999-99-99';if(k==='nom')return (p.nom||'').toLowerCase();return p[k]||0;}
function renderProjets(){
 const rows=(D.projets||[]).filter(p=>state.sec.has(p.s)&&state.par.has(p.p)&&state.st.has(p.t))
  .filter(p=>!PROJ_Q||(p.nom||'').toLowerCase().includes(PROJ_Q)||(p.code||'').toLowerCase().includes(PROJ_Q));
 rows.sort((a,b)=>{const x=pval(a,PROJ_K),y=pval(b,PROJ_K);return (x<y?-1:x>y?1:0)*PROJ_DIR;});
 const tp=rows.reduce((s,p)=>s+p.prev,0), td=rows.reduce((s,p)=>s+p.dec,0);
 $('proj-count').textContent=rows.length+' projet(s) · '+f2(tp)+' Mds engagés · '+f2(td)+' Mds décaissés';
 $('t-projets').innerHTML=rows.map(p=>{const t=p.prev?p.dec/p.prev*100:0;const late=p.fin&&p.fin<='2027-08-28';
  return '<tr><td><div class="tt">'+p.nom+'</div><div class="code">'+(p.code||'')+'</div></td><td>'+D.secteurs[p.s]+'</td><td>'+D.partenaires[p.p]+'</td><td class="tnum">'+f2(p.prev)+'</td><td class="tnum">'+f2(p.dec)+'</td><td class="tnum"><span class="progress"><i style="width:'+Math.min(100,t)+'%"></i></span>'+NB0.format(t)+' %</td><td class="tnum" style="'+(late?'color:var(--risque);font-weight:700':'')+'">'+(p.fin?p.fin.slice(8,10)+'/'+p.fin.slice(5,7)+'/'+p.fin.slice(0,4):'—')+'</td></tr>';}).join('')||'<tr><td colspan="7" class="empty">Aucun projet.</td></tr>';
}
function renderRecos(){
 let list=[];
 if(RECO_FULL&&RECO_FULL.length){list=RECO_FULL.map(r=>({txt:r.texte||r.recommandation||r.txt||'',src:r.partenaire||r.src||'',ech:String(r.echeance||r.ech||''),fait:r.executee||r.fait||0}));}
 else{list=(D.projets||[]).filter(p=>state.sec.has(p.s)&&state.par.has(p.p)&&state.st.has(p.t)&&p.fin).map(p=>({txt:'Préparer la clôture et justifier le solde de « '+p.nom+' »',src:D.partenaires[p.p],ech:String(p.fin),fait:0}));}
 if(state.par.size<D.partenaires.length){const sel=Array.from(state.par).map(i=>D.partenaires[i]);list=list.filter(r=>sel.some(n=>(r.src||'').indexOf(n)>=0));}
 list=list.map(r=>{const d=r.ech.slice(0,10);return Object.assign({},r,{ret:d?Math.round((new Date('2026-08-28')-new Date(d))/864e5):0});});
 const act=list.filter(r=>!r.fait).sort((a,b)=>b.ret-a.ret);
 $('reco-count').textContent=act.length+' active(s) · '+(list.length-act.length)+' levée(s)';
...
function renderPartenaires(){
 const rows=D.partenaires.map((nm,i)=>{const r={prev:0,dec:0,n:0,sec:{}};(D.cells||[]).forEach(c=>{if(c.p===i&&state.sec.has(c.s)&&state.st.has(c.t)){r.prev+=c.prev;r.dec+=c.dec;r.n+=c.n;r.sec[c.s]=(r.sec[c.s]||0)+c.prev;}});return Object.assign({nm:nm},r);}).filter(r=>r.n>0).sort((a,b)=>b.prev-a.prev);
 $('par-cards').innerHTML=rows.map(r=>{const t=r.prev?r.dec/r.prev*100:0;
  const tops=Object.keys(r.sec).map(k=>({k:+k,v:r.sec[k]})).sort((a,b)=>b.v-a.v).slice(0,3);
  return '<div class="card"><div class="cardhead"><h4>'+r.nm+'</h4><span class="unit">'+r.n+' projet(s)</span></div><div class="figs" style="grid-template-columns:1fr 1fr;margin-top:8px"><div class="fig"><div class="lb">Engagé</div><div class="vl">'+f2(r.prev)+'<small>Mds</small></div></div><div class="fig"><div class="lb">Décaissé</div><div class="vl">'+f2(r.dec)+'<small>Mds</small></div></div></div><div class="gauge" style="margin-top:12px"><i style="width:'+Math.min(100,t)+'%"></i></div><div class="note" style="border:0;margin-top:8px">Taux d\u2019exécution : <b>'+NB1.format(t)+' %</b> · Solde : <b>'+f2(r.prev-r.dec)+' Mds</b><br>Secteurs principaux : '+tops.map(x=>D.secteurs[x.k]).join(', ')+'</div></div>';}).join('')||'<div class="card"><div class="empty">Aucune donnée.</div></div>';
}
function refreshView(){if(CUR==='projets')renderProjets();else if(CUR==='recos')renderRecos();else if(CUR==='revues')renderRevues();else if(CUR==='partenaires')renderPartenaires();}
function switchView(v){CUR=v;
 document.querySelectorAll('#tabs .tab').forEach(b=>b.classList.toggle('on',b.getAttribute('data-v')===v));
 ['dash','projets','recos','revues','partenaires'].forEach(k=>{const el=$('view-'+k);if(el)el.hidden=(k!==v);});
 refreshView();
...
document.addEventListener('click',e=>{
 ...
 if(b){switchView(b.getAttribute('data-v'));return;}
 const th=e.target.closest('#t-projets th[data-k]');
 if(th){const k=th.getAttribute('data-k');if(PROJ_K===k)PROJ_DIR*=-1;else{PROJ_K=k;PROJ_DIR=-1;}renderProjets();}
});
document.addEventListener('input',e=>{if(e.target&&e.target.id==='proj-search'){PROJ_Q=e.target.value.trim().toLowerCase();renderProjets();}});
$('btn-reset').onclick=()=>{
...
async function init(){
 ...
 try{const r=await fetch('/api/suivi/revues');if(r.ok){const dd=await r.json();if(Array.isArray(dd)&&dd.length)REVUES_FULL=dd;render();}}catch(e){}
}
init();
</script>
</body>
