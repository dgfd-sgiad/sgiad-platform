...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('/api/session/info')
        ...
        .then(data => {
            ...
            if (connexionEl) {
                const userLine = data.user ? `Utilisateur : ${data.user} — ` : '';
                const derniere = data.derniere_connexion
                    ? `Dernière connexion : ${data.derniere_connexion}`
                    : 'Première connexion enregistrée';
                connexionEl.textContent = `${userLine}${derniere} — Nombre total de connexions : ${data.total_connexions}`;
            }

            const userEl = document.getElementById('infoUtilisateur');
            if (userEl && data.user) {
                userEl.textContent = `Utilisateur : ${data.user}`;
            }
        })
        .catch(() => {
            const connexionEl = document.getElementById('infoConnexion');
